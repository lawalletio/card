...
describe('POST /card', () => {
  ...
  it('should fail for invalid request', async () => {
    jest
      ...
      .mockReturnValue({ tags: [['t', 'invalidtag']] } as NostrEvent);
  ...
  describe('Card activation', () => {
    ...
    ])('should fail for invalid body', async (reqEvent: NostrEvent) => {
      jest.mocked(parseEventBody).mockReturnValue({
        ...reqEvent,
        tags: [['t', 'card-activation-request']],
      } as NostrEvent);
